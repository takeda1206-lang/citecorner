...
### 日本語文献対応（2026-07-07 追加・ユーザー指定仕様）

- **日本語（かな・カナ・漢字・半角カナ）を含む入力は検索を一切しない**（英語DBの誤ヒット防止）。
  入力された範囲だけで Vancouver 風に整形。**推測もしない**。足りない項目は出力から省く。
- 解釈は「著者名. タイトル. 雑誌名. 年;巻(号):頁.」の**位置ベースの機械的分割**
  （`src/lib/parse.js` の `parseJapaneseCitation`）。全角数字・記号・全角ダッシュ・読点は正規化。
  入力に DOI/PMID が書かれていればそのまま拾う（検索はしない）。
- 整形結果の上に**「日本語文献の修正欄」**カード（項目別: 著者/タイトル/雑誌名/年/巻/号/開始頁/終了頁/DOI/PMID）
  ＋**「情報を反映」ボタン**。反映すると整形結果が更新される（`JapaneseEditCard.jsx`）。
- 出力は英語と同じ5種。**筆頭著者+et al. は日本語では「武田太郎, 他.」形式**（ユーザー選択）。
  著者1人なら「他」を付けない。著者欄の「他/ほか/et al」トークンは著者として数えない。
- 日本語のとき「取得した情報」カードは非表示（修正欄が項目一覧を兼ねる）。
- 種類チップに「日本語」を追加（自動判別の誤り時に強制指定可能）。
- 画像OCRタブのテキスト欄に日本語を貼った場合も同様に検索なし整形
...
└── src/
    ...
    │   ├── InputPanel.jsx          # 3タブ切替（テキスト/PDF/画像OCR）+ ページ全体D&D
    │   ├── SmartInput.jsx          # ★自動判別+自動検索+種類チップ+Enter即実行
    │   ├── JapaneseEditCard.jsx    # ★日本語文献の修正欄（項目別編集+情報を反映）
...
| 2026-07-07 | 日本語文献 | **日本語を含む入力は検索なし・推測なし**、入力の範囲のみで整形。修正欄は**項目ごとの欄**＋「情報を反映」。et al. 相当は**「武田太郎, 他.」** |
...
- [x] ~~Cloudflare Pages への実デプロイ~~ → **公開済み: https://citecorner.pages.dev**（2026-07-07、コミット 8069bf6＝使い方ページまで反映済み）
  - 使い方ページは https://citecorner.pages.dev/usage （Pages の仕様で /usage.html は /usage へ308リダイレクトされる。アプリのリンクはそのまま動く）
- [x] ~~改修分のコミット~~ → コミット・push済み
...
- **本番URL**: https://citecorner.pages.dev
- **方式**: GitHub連携（`git@github.com:takeda1206-lang/citecorner.git` の main へ push → Cloudflare Pages が自動ビルド・自動デプロイ）
- **再公開の手順**: 変更をコミットして `git push origin main` するだけ。
  数分でビルドされ本番に反映（`npx wrangler pages deployment list --project-name=citecorner` で確認可能）
- リモートはSSH（HTTPSは認証情報が無くpush不可だったためSSHに切替済み。鍵は `~/.ssh/id_ed25519`）
- Pages Function（`/api/ncbi` プロキシ）も本番で動作確認済み

| ビルド設定（Cloudflare側・設定済み） | 値 |
|---|---|
| Build command | `npm run build` |
| Build output directory | `dist` |
| Functions | `functions/` を自動検出 |
| 環境変数 / シークレット | **不要** |
